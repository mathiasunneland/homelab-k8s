locals {
  ...
  nginx_values = templatefile("nginx_values.yaml", {
    nginx_lb_ip = var.nginx_lb_ip
  })

  postgres_values = templatefile("postgres_values.yaml", {
    postgres_database = var.postgres_database
    postgres_username = var.postgres_username
    postgres_password = var.postgres_password
  })

  redis_values = templatefile("redis_values.yaml", {
    redis_password = var.redis_password
  })

  chess_live_ingress = templatefile("chess_live_ingress.yaml", {
  ...
  chess_live_secrets = templatefile("chess_live_secrets.yaml", {
