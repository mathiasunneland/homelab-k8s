...
resource "proxmox_virtual_environment_file" "k8s_worker_cloud_init" {
  ...
  source_raw {
    data      = local.k8s_worker_cloud_init
    file_name = "k8s_worker_cloud_init.yaml"
  }
}
...
resource "proxmox_virtual_environment_vm" "k8s_worker" {
  ...
  initialization {
    ...
    ip_config {
      ipv4 {
        address    = "${var.k8s_worker_ip_start}.${count.index + 234}/24"
        gateway    = var.gateway_ip
      }
    }

    user_data_file_id = proxmox_virtual_environment_file.k8s_worker_cloud_init.id
  }
}
...
# }
#
# resource "null_resource" "copy_files_to_k8s_worker" {
#   depends_on = [
...
#     proxmox_virtual_environment_file.k8s_worker_cloud_init,
